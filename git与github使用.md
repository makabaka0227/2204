...
![1571977539957](/1571977539957.png)
...
# 如果是一个已经暂存过的文件，可以快速提交，如果是未追踪(新添加)的文件，那么命令将不生效。
git commit -a -m '提交说明'
...
![1572056608832](/1572056608832.png)

### git config配置

**如果是第一次提交，需要配置提交者信息**，推荐和github的账号邮箱一致

如果第一次 执行 gitcommit 会提示,配置用户名和邮箱
...
- `git branch`可以查看所有的分支，注意一定要有提交操作

- 在当前分支的前面会有一个`*`

- 在git中，有一个特殊指针`HEAD`,永远会指向当前分支

  ![1572073577784](/1572073577784.png)
...
![1572073696372](/1572073696372.png)

如果在主分支下,创建其它分支,那么主分支下的文件都会被克隆一份给其它分支.
...
让本地仓库和github或gitee建立连接的第一步就执行git clone,只有第一次的时候执行

- 作用：克隆远程仓库的代码到本地
- git clone [远程仓库地址]   第一次执行
- `git clone git@github.com:happyChengfeng/2108.git 本地项目名    会在本地新建一个`test`文件夹，在test中包含了一个`.git`目录，用于保存所有的版本记录，同时test文件中还有最新的代码，你可以直接进行后续的开发和使用。
...
- `git push 仓库地址 main` 在代码提交到远程仓库，注意master分支必须写，不能省略

- 例子：`git push git@github.com:happyChengfeng/2108.git  main` 如果第一次使用，需要填写github的用户名和密码

- 

  git push origin master -f   强制进行如果git仓库有内容，这个指令会把原来的内容覆盖掉
...
git remote add 2002 git@github.com:happyChengfeng/2108.git
...
1 获取git地址  执行 git clone,不要执行git init

2 创建分支,方便自己在本地操作

3 分支测试完成,合并到主分支

4 [先执行pull,获取最新代码],在执行push 推送



5 确定leader帮你的git用户名添加到了,github仓库白名单中,否则不能提交
